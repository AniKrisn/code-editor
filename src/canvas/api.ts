...
export function createEditorAPI(editor: Editor) {
	return {
		 ...
		 * @param updates - Array of shape updates (each must include id and type)
		 *
		 * @example
		 * api.updateShapes([
		 *   { id: shape1Id, type: 'geo', x: 100, y: 100 },
		 *   { id: shape2Id, type: 'arrow', props: { color: 'red' } }
		 * ])
		 */
		updateShapes(updates: { id: TLShapeId; type: string; [key: string]: unknown }[]): void {
		...
		/**
		 * Lock canvas interaction to prevent selection and panning.
		 * Useful for interactive examples that handle their own input.
		 *
		 * @example
		 * canvas.lockInteraction()
		 */
