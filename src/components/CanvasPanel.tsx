...
function CustomHandles({ children }: TLHandlesProps) {
  ...
  const shouldDisplayDefaultHandles = useValue(
    ...
    () => {
      ...
      if (
        onlySelectedShape &&
        editor.isShapeOfType(onlySelectedShape, "bezier-curve")
      ) {
      ...
      if (editor.isInAny("select.editing_shape")) {
        ...
        return (
          onlySelectedShape && editor.isShapeOfType(onlySelectedShape, "note")
        );
      }
      return false;
    },
...
const customShapeUtils = [WatercolorShapeUtil];

/**
 * Auto-recovering error fallback that clears bad state and remounts.
 */
function RecoveringErrorFallback({
  error,
}: {
  error: unknown;
  editor?: Editor;
