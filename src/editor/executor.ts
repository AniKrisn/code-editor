...
// Track active intervals and timeouts created by user code
...
/**
 * Clear all active intervals and timeouts created by user code.
 */
...
export async function executeCode(code: string, editor: Editor): Promise<ExecutionResult> {
	...
	// Clear any existing intervals/timeouts from previous code execution
	clearActiveTimers()

	// Create a restore point before executing user code
	const markId = editor.markHistoryStoppingPoint('code-execution')

	// Create wrapped versions of setInterval and setTimeout that track their IDs
	const trackedSetInterval = (callback: () => void, delay?: number, ...args: unknown[]) => {
		const id = setInterval(callback, delay, ...args) as unknown as number
	...
	try {
		...
		editor.run(() => {
			...
			// Provide tracked timer functions instead of raw window functions
